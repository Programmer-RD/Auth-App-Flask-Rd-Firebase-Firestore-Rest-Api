...
class Sign_Up:
    ...
    def check_email_and_password(self):
        try:
            ...
            for doc_ref_ in doc_ref:
                passwords.append(doc_ref_.to_dict())
            doc_ref = (
                db.collection("auth rest api").where("email", "==", self.email).stream()
            )
            emails = []
            for doc_ref_ in doc_ref:
    ...
    def add_to_db(self):
        su = Sign_Up(email=self.email, password=self.password, user_name=self.password)
        results = [
        ...
        if False not in results:
            doc_ref = db.collection("auth rest api").document()
...
class Sign_In:
    ...
    def check_email_and_password(self):
        ...
        doc_ref = (
            db.collection("auth rest api")
            .where("email", "==", self.user_name_or_email)
            .stream()
        )
        emails = []
        for doc_ref_ in doc_ref:
